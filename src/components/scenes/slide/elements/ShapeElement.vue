...
<template>
  <div class="element-content shape-element">
    <svg width="100%" height="100%" :viewBox="`0 0 ${viewBox[0]} ${viewBox[1]}`" preserveAspectRatio="none">
      <defs v-if="element.gradient && element.gradient.type === 'linear'">
        ...
        >
          <stop v-for="(c, i) in element.gradient.colors" :key="i" :offset="`${c.pos}%`" :stop-color="c.color" />
        </linearGradient>
      </defs>
      <path
        :d="element.path"
...
const fill = computed(() => (props.element.gradient ? `url(#shape-grad-${props.element.id})` : props.element.fill));
