 ...
 */
import type { ActionEngine } from '#/core/action/engine';
import type { AudioPlayer } from '#/core/audio/audio-player';
import { isTTSProviderEnabled } from '#/core/audio/provider-enablement';
import { DISCUSSION_TRIGGER_DELAY_MS, estimateSpeechDurationMs, resolvePlaybackCursor } from '#/core/choreography';
import { createLogger } from '#/core/logger';
import { useCanvasStore } from '#/stores/canvas';
import { useSettingsStore } from '#/stores/settings';
import type { Action, DiscussionAction, SpeechAction } from '#/types/action';
import type { Scene } from '#/types/stage';

import { canJumpWithinReconstructablePrefix, isWhiteboardPlaybackAction } from './action-navigation';
import type { Effect, EngineMode, PlaybackEngineCallbacks, PlaybackSnapshot, TopicState, TriggerEvent } from './types';
...
export class PlaybackEngine {
  ...
  canJumpToAction(actionIndex: number): boolean {
    ...
    return this.mode !== 'live' && canJumpWithinReconstructablePrefix(actions, this.actionIndex, actionIndex);
  }
          ...
          .then((audioStarted) => {
            ...
            if (!audioStarted) {
              ...
              if (
                ...
                isTTSProviderEnabled('browser-native-tts', settings.ttsProvidersConfig?.['browser-native-tts']) &&
  ...
  private async playBrowserTTSChunk(generation: number): Promise<void> {
    ...
    if (!voiceFound) {
      ...
      const cjkRatio =
        chunkText.length > 0 ? (chunkText.match(/[\u4E00-\u9FFF\u3400-\u4DBF]/g) || []).length / chunkText.length : 0;
