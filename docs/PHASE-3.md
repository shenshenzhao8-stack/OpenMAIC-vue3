...
3. 编写 `usePlaybackEngine` hook（组合式函数），把纯 TS 引擎接到 Vue 响应式状态；
...
| `src/hooks/useScene.ts` | `lib/contexts/scene-context.tsx` | SceneContext + useSceneSelector | **改写**：React Context → Vue provide/inject + computed |
...
| `src/hooks/usePlaybackEngine.ts` | `components/edit/PlaybackChromeRoot.tsx` | 创建引擎并接回调 | **改写（简化）**：mode/lectureSpeech 响应式；play/pause/resume/stop/翻页；onSceneChange → setCurrentSceneId |
