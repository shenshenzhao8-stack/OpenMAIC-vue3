...
- 需求规划：
  ...
  4. `usePlaybackEngine` composable：把 PlaybackEngine 回调接到 Vue 响应式状态；
...
- 需求规划：
  ...
  3. `useDiscussionTTS` composable（React→Vue）：语音队列 + 封口入队 + shouldHold；
  4. 字幕接线：`onSpeechStart` → 逐字揭示；`segmentDone` 放行。
- 原项目文件对照：`engine.ts`（已搬）、`audio-player.ts`（已搬）、`timing.ts`（已搬）、
  `lib/hooks/use-discussion-tts.ts`（改写 composable）、`stream-buffer.ts`（已搬）。
...
- 需求规划：
  1. `useChatSession` composable（改写自 `use-chat-sessions.ts`）：sendMessage、
     SSE→buffer 翻译器、打断补"..."；
